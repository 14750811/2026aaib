...
void setup() { //設定
  size(300, 400); //視窗大小
  img=loadImage("gopher.png");//把gopher.png拉到程式裡
}
void mousePressed() {
...
void draw() {
  ...
  if (frameCount%60==0) { //每60frames(1秒)冒出一隻地鼠
    int i=int(random(3)), j=int(random(3)); //亂數決定 i j 值
    a[i][j]=60; //會亂數決定1格，站起來60frames(1秒)
  }
  for (int i=0; i<3; i++) { //左手i
    for (int j=0; j<3; j++) { //右手j
      float x=j*100, y=i*100+100; //推算座標，小心y往下100(藏白色板後)
      //if(i==0 && j==0)y+=mouseY; //暫時實驗用，看到畫面壞了
      if (a[i][j]>0){ //如果有地鼠
        a[i][j]-=1; //生命的倒數計時
